...

{
   ...
   Arrow (int x, int y, int dx, int dy) // this constructor imports the images of 4 different direction arrows and assigns them to images 1, 2, 3 and 4
  {
    ...
    image1 = loadImage("DownRight.jpg"); // loads an image
    image1.resize(size,size); // resizes the image to the value of the size variable (global)
    
    image2 = loadImage("DownLeft.jpg");
    image2.resize(size,size);
    
    image3 = loadImage("UpRight.jpg");
    image3.resize(size,size);
    
    image4 = loadImage("UpLeft.jpg");
  ...
  void render() // renders the image
  {
      ...
      if (speedY == abs(speedY) && speedX == abs(speedX)) //if the x and y are positive the arrow faces diagonally to the bottom right
      {
        image(image1,x,y);
      }
      
      else if (speedY == abs(speedY) && speedX != abs(speedX)) //if the x is negative and y is positive the arrow faces diagonally to the bottom left
      {
        image(image2,x,y);
      }
      
      else if (speedY != abs(speedY) && speedX == abs(speedX)) //if the x is positive and the y is negative the arrow faces diagonally to the top right
      {
        image(image3,x,y);
      }
      
      else if (speedY != abs(speedY) && speedX != abs(speedX)) //if the x and y are negative the arrow faces diagonally to the top left
      {
        image(image4,x,y);
      }
 ...
 void update() // runs the procedures it contains at the same time
{
  ...
  boolean crash(Arrow other) // this detects if 2 arrows are in contact with each other
   ...
   void wall() // this detects if an arrow is in contact with a wall and inverts it if it is
   {
     ...
     if (this.y > width - size || this.y < size) // this checks if the object is touching the vertical walls on the top or bottom of the canvas and inverts the vertical speed if it is
     {
     speedY = speedY*-1;
     }
   }  
